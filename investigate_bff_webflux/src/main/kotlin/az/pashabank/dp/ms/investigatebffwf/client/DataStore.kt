...
class DataStore {
    ...
    final val customers: List<Customer>
    final val accounts: List<BankAccount>
    final val addresses: List<Address>
    final val transactions: List<Transaction>
    final val contacts: List<Contact>
    ...
    init {
        ...
        customers = baseCustomers + multiply(baseCustomers, 100) { c, i ->
            c.copy(id = "${c.id}_$i", email = "${c.email}_$i")
        }
        accounts = baseAccounts + multiply(baseAccounts, 100) { a, i ->
            a.copy(id = "${a.id}_$i", customerId = "${a.customerId}_$i", accountNumber = "${a.accountNumber}_$i")
        }
        addresses = baseAddresses + multiply(baseAddresses, 100) { a, i ->
            a.copy(id = "${a.id}_$i", customerId = "${a.customerId}_$i")
        }
        transactions = baseTransactions + multiply(baseTransactions, 100) { t, i ->
            t.copy(id = "${t.id}_$i", accountId = "${t.accountId}_$i")
        }
        contacts = baseContacts + multiply(baseContacts, 100) { c, i ->
            c.copy(id = "${c.id}_$i", customerId = "${c.customerId}_$i")
        }
